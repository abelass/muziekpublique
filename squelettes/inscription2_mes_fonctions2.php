...
function inscription2_verifier_tables(){
	...
	spip_query("CREATE TABLE IF NOT EXISTS ".$table_nom." (id bigint NOT NULL AUTO_INCREMENT PRIMARY KEY, id_auteur bigint NOT NULL, FOREIGN KEY (id_auteur) REFERENCES spip_auteurs (id_auteur), idx enum('', '1', 'non', 'oui', 'idx') DEFAULT '' NOT NULL);");
	
	//Attention oubli dans les version anterieures en 1.9.2 => Permet de faire fonctionner les crayons
	if (!isset($desc['field']['idx'])){
		spip_query("ALTER TABLE ".$table_nom." ADD idx enum('', '1', 'non', 'oui', 'idx') DEFAULT '' NOT NULL");
		$desc['field']['idx'] = "enum('', '1', 'non', 'oui', 'idx') DEFAULT '' NOT NULL";
	}
