...
function Agenda_action_update_repetitions($id_evenement,$repetitions,$liste_mots){
	...
	if ($row = spip_fetch_array(($res))){
		...
		$prix_membres = $row['prix_membres'];	
		$prix_non_membres = $row['prix_non_membres'];
		$prix_prevente_membres = $row['prix_prevente_membres'];
		$prix_prevente_non_membres = $row['prix_prevente_non_membres'];
		...
		while ($row = spip_fetch_array($res)){
			...
			if (in_array($date,$repetitions)){
				...
				. ",`prix_membres`="._q($prix_membres)
				. ",`prix_non_membres`="._q($prix_non_membres)
				. ",`prix_prevente_membres`="._q($prix_prevente_membres)
				. ",`prix_prevente_non_membres`="._q($prix_prevente_non_membres)
		...
		foreach($repetitions as $date){
			if (!in_array($date,$repetitions_updated)){
				...
				$update_descriptif = $descriptif;
				$update_lieu = $lieu;
				$update_prix_membres = $prix_membres;
				$update_prix_non_membres = $prix_non_membres;
				$update_prix_prevente_membres = $prix_prevente_membres;
				$update_prix_prevente_non_membres = $prix_prevente_non_membres;
...
function Agenda_action_formulaire_article($id_article,$id_evenement, $c=NULL){
	...
	if (($insert || $modif)){
		...
		$prix_membres = _request('evenement_prix_membres',$c);
		$prix_non_membres = _request('evenement_prix_non_membres',$c);
		$prix_prevente_membres = _request('evenement_prix_prevente_membres',$c);
		$prix_prevente_non_membres = _request('evenement_prix_prevente_non_membres',$c);
