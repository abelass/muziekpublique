...
function i2_import_table_fields($tables,$type='unique'){
	...
	$table_fields_final = array();
		...
		$table_desc = $trouver_table($tables);
		

		
		$activites=array_keys(is_array($table_desc['field']) ? $table_desc['field'] : array());
		$activites=array_flip($activites);
		foreach ($activites as $key=>$value) {
				$table_fields_final [$key] = $key;
		}
