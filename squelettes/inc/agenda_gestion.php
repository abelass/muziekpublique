...
function Agenda_formulaire_article_afficher_evenements($id_article, $flag_editable, $script)
{
	...
	. " GROUP BY evenements.id_evenement ORDER BY evenements.date_debut DESC");
...
function Agenda_formulaire_edition_evenement($id_evenement, $neweven, $ndate="", $titre_defaut=""){
	...
	if ($id_evenement!=NULL){
		...
		if ($row = spip_fetch_array($res)){
			if (!$neweven){
				...
				$prixmembres=entites_html($row['prix_membres']);	
				$prixnonmembres=entites_html($row['prix_non_membres']);
				$prixpreventemembres=entites_html($row['prix_prevente_membres']);
				$prixpreventenonmembres=entites_html($row['prix_prevente_non_membres']);
	...
	$res2 = spip_query("SELECT * FROM spip_articles WHERE id_article=".$id_article);
		while($row2 = spip_fetch_array($res2)){
		$id_rubrique = $row2['id_rubrique'];
		};
			// Affichage dans les sections concerts
	if ($id_rubrique==8 or $id_rubrique==26 or $id_rubrique==39) {
		$out .=  "<div class='prix-titre'>"._T('agenda:prix_membres')."</div>";
		$out .=  "<div class='prix-visu'>";
		$out .=  "<input type='text' name='evenement_prix_membres' value=\"$prixmembres\" style='width:50px;' />";
		$out .=  " EUR</div>\n";
		$out .=  "<div class='prix-titre'>"._T('agenda:prix_non_membres')."</div>";
		$out .=  "<div class='prix-visu'>";
		$out .=  "<input type='text' name='evenement_prix_non_membres' value=\"$prixnonmembres\" style='width:50px;' />";
		$out .=  " EUR</div>\n";
		$out .=  "<div class='prix-titre'>"._T('agenda:prix_prevente_non_membre')."</div>";
		$out .=  "<div class='prix-visu'>";
		$out .=  "<input type='text' name='evenement_prix_prevente_non_membres' value=\"$prixpreventenonmembres\" style='width:50px;' />";
		$out .=  " EUR</div>\n";
	}
			// Affichage dans les sections cours
	elseif ($id_rubrique==1 or $id_rubrique==27 or $id_rubrique==40) {
		$out .=  "<div class='prix-titre'>"._T('prix_annee')."</div>";
		$out .=  "<div class='prix-visu'>";
		$out .=  "<input type='text' name='evenement_prix_prevente_non_membres' value=\"$prixpreventenonmembres\" style='width:50px;' />";
		$out .=  " EUR</div>\n";
		$out .=  "<div class='prix-titre'>"._T('prix_periode1')."</div>";
		$out .=  "<div class='prix-visu'>";
		$out .=  "<input type='text' name='evenement_prix_membres' value=\"$prixmembres\" style='width:50px;' />";
		$out .=  " EUR</div>\n";
		$out .=  "<div class='prix-titre'>"._T('prix_periode2')."</div>";
		$out .=  "<div class='prix-visu'>";
		$out .=  "<input type='text' name='evenement_prix_non_membres' value=\"$prixnonmembres\" style='width:50px;' />";
		$out .=  " EUR</div>\n";
	};
