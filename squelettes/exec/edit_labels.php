<?php
	...
	**/
	...
	function exec_edit_labels(){
		...
		$id_requete  =_request('id_requete');
		...
		// Menu de sélection statut interne
		...
		// Menu de sélection requetes
		echo '<td style="text-align:right;">';
		echo '<form method="post" action="#">';
		echo '<select name ="statut_interne" class="fondl" onchange="form.submit()">';
		$query = spip_query ( "SELECT * FROM spip_requetes  WHERE type='etiquettes'"   );
		while ($data = spip_fetch_array($query)) {	
			echo '<option value="'.$data['id_requete'].'"';
			if ($id_requete==$data['id_requete']) {echo ' selected="selected"';}
			echo '> '.$data['nom_requete'].'</option>';
		};
	...
	if ($id_requete){
	$query = spip_query ( "SELECT * FROM spip_requetes  WHERE id_requete='.$id_requete.'"   );
		while ($data = spip_fetch_array($query)) {	
			$selection=$data['selection'];
		};
	}
	else{
	$query = spip_query ( "SELECT * FROM spip_requetes  WHERE defaut='defaut' AND type='etiquettes'"   );
		while ($data = spip_fetch_array($query)) {	
			$selection=$data['selection'];
		};
	};
		...
		echo '</tr>';
		$query = spip_query ("SELECT * FROM spip_auteurs_elargis  WHERE statut_interne like '$statut_interne' ORDER BY nom_famille DESC" );
		while ($data = spip_fetch_array($query))  {
